...
class Router {
    public static function redirect($uri, $method, $body_fields, $get_fields, $header_fields){
        ...
        $token_validation = self::check_headers($header_fields);
    ...
    public static function check_headers($header_fields){
        $response = (object)[
            "status"=> false,
            "code"=> 401,
            "msg"=> "ApiToken is missing"
        ];
        if(!empty($header_fields->ApiToken??null)){
            $ApiToken = $header_fields->ApiToken;
            if(!UserController::check_token($ApiToken)){
                $response->msg = "invalid ApiToken";
            } else {
                $response->status = true;
                $response->code = 200;
                $response->msg = "valid ApiTonken";
            }
        }
        return $response;
    }
